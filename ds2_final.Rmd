...
rpart.fit <- train(recovery_time ~ . , 
                   trainData, 
                   method = "rpart",
                   tuneGrid = data.frame(cp = exp(seq(-12,-2.5, length = 10))),
                   trControl = ctrl)
...
ctree.fit <- train(recovery_time ~ . , 
                   trainData, 
                   method = "ctree",
                   tuneGrid = data.frame(mincriterion = 1-exp(seq(-1, -6, length = 10))),
                   trControl = ctrl)
...
```{r, warning=FALSE}
glmnGrid <- expand.grid(.alpha = seq(0, 5, length = 10),
...
ctree.fit.binary <- train(recovery_time ~ . , 
                          ...
                          method = "ctree",
                          tuneGrid = data.frame(
                            mincriterion = 1-exp(seq(-1, 0, length = 10))),
                          metric = "ROC",
                          trControl = ctrl_binary)
...
# Boosting
```{r}
gbmA.grid.binary <- expand.grid(n.trees = c(1000,2000,3000,4000,5000),
