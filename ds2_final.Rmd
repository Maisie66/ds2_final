...
ctree.fit <- train(recovery_time ~ . , 
                   trainData, 
                   method = "ctree",
                   tuneGrid = data.frame(mincriterion = 1-exp(seq(-6, 1, length = 50))),
                   trControl = ctrl)
ggplot(ctree.fit, highlight = TRUE)

resample <- resamples(list(ctree = ctree.fit,
                         rpart = rpart.fit))
summary(resample)
...
```{r boosting_gbm}
set.seed(6360)
gbm.grid <- expand.grid(n.trees = c(700, 1000, 2000, 2100, 2200, 2500, 2700, 3000, 4000),
